...
public class JmhBenchmarkRunner {
  ...
  public static List<BenchmarkParameters> fillBenchmarkParameters(JSONArray locks, JSONArray array)
    ...
    for (Object o : array) {
      ...
      if (obj.get("limitThreads") != null) {
        String limitThreads = (String) obj.get("limitThreads");
        if (limitThreads.equals("ONLY_OVERSUB")) {
          threads = threads.stream().filter(it -> it >= Runtime.getRuntime().availableProcessors())
              .collect(Collectors.toList());
        } else if (limitThreads.equals("ONLY_UNDERSUB")) {
          threads = threads.stream().filter(it -> it < Runtime.getRuntime().availableProcessors())
              .collect(Collectors.toList());
        }
      }
      if (obj.get("threadsFrom") != null) {
        long threadsFrom = (long) (obj.get("threadsFrom"));
        threads = threads.stream().filter(it -> it >= threadsFrom)
            .collect(Collectors.toList());
      }
      ...
      int yieldsBefore = 1;
      if (obj.get("yieldsBefore") != null) {
        yieldsBefore = (int) ((long) obj.get("yieldsBefore"));
      }
      ...
      if (name.equals("consumeCpu")) {
        ...
        boolean limitVirtualScheduler =
            obj.get("limitVirtualScheduler") == null || ((Boolean) obj.get("limitVirtualScheduler"));
        ...
        double beforeConsumeCpuTokensTimeNanos =
            JmhConsumeCpuTokensUtil.estimateConsumeCpuTokensTimeNanos(before);
        double inConsumeCpuTokensTimeNanos = JmhConsumeCpuTokensUtil.estimateConsumeCpuTokensTimeNanos(in);
        double highContentionWithoutLocksNanos =
            JmhConsumeCpuTokensUtil.estimateHighContentionWithoutLocksTimeNanos(before, in, actionsCount);
  ...
  public static BenchmarkResultsCsv runBenchmark(BenchmarkParameters parameters) throws RunnerException {
    switch (parameters) {
      case ConsumeCpuBenchmarkParameters param -> {
        ...
        if (param.limitVirtualScheduler) {
          options = options.jvmArgsAppend(
              "-Djdk.virtualThreadScheduler.parallelism=" +
                  Math.min(Runtime.getRuntime().availableProcessors(), param.threads)
        ...
        try {
          withLocksNanos = runBenchmarkNano(options,
              Map.of(
                  ...
                  "yieldsBefore", Integer.toString(param.yieldsBefore)
              )
          );
        } catch (Exception e) {
        ...
        return new BenchmarkResultsCsv(parameters.getBenchmarkName(), parameters.lockType.name(),
            parameters.threads,
            overheadNanosMax, overheadNanosMin, overheadNanosAverage, throughputNanosMax, throughputNanosMin,
            throughputNanosMedian);
      }
